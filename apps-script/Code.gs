...
function doGet(e) {
  try {
    ...
    const history = params.history ? Number(params.history) : 0;
    const sheet = getSheet_();
    const lastRow = sheet.getLastRow();

    if (lastRow < 2) {
      return jsonOutput_(history > 0 ? { items: [] } : {});
    }

    if (history > 0) {
      const rowCount = Math.min(history, lastRow - 1);
      const startRow = lastRow - rowCount + 1;
      const values = sheet.getRange(startRow, 1, rowCount, 4).getValues();
      ...
      return jsonOutput_({ items: items });
    }
